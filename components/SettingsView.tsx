...
const SettingsView: React.FC<SettingsViewProps> = ({ fbConfig, setFbConfig }) => {
  ...
  const updateModelField = (id: string, field: keyof ModelConfig, value: string | boolean) => {
  ...
  const addModel = () => {
    if (!newModel.name || !newModel.modelString) {
      showToast('Please fill in all required fields', 'error');
      return;
    }

    const id = `${newModel.provider}-${Date.now()}`;
    setModels(prev => [...prev, {
      id,
      provider: newModel.provider as 'gemini' | 'openai' | 'openrouter' | 'huggingface',
      name: newModel.name,
      modelString: newModel.modelString,
      apiKey: 'Configured via environment variable',
      isActive: true
    }]);
