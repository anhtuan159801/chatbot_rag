...
// Mock function for system health analysis
const analyzeSystemHealth = async (metrics: any, documents: any) => {
  // Simulate API delay
  await new Promise((resolve) => setTimeout(resolve, 2000));
  return "Hệ thống hoạt động ổn định. Tất cả các chỉ số trong phạm vi bình thường. Không có vấn đề nghiêm trọng nào được phát hiện.";
};
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            Theo dõi hiệu suất và hoạt động của hệ thống RAG
          </p>
        </div>
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold flex items-center gap-2 text-slate-800">
                <div
                  className="p-1.5 bg-blue-100 rounded-lg text-blue-700"
                  aria-hidden="true"
                >
                  <Sparkles size={20} />
                </div>
              ...
              {analysis ? (
                ...
                <div className="flex flex-col items-center justify-center h-full text-slate-400 py-4">
                  ...
                  <p className="text-sm text-center">
                    Nhấn nút "Phân tích nhanh" để nhận báo cáo về tình trạng
                    vector, độ trễ và tải hệ thống.
                  </p>
                </div>
              )}
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <span id="metric1" className="text-slate-500 font-semibold text-sm">
              Văn bản pháp quy
            </span>
            <div
              className="p-2 bg-indigo-100 rounded-lg text-indigo-700"
              aria-hidden="true"
            >
              <Database size={20} />
            </div>
          ...
          <div
            ...
            aria-label={`Tổng số văn bản pháp lý: ${totalVectors.toLocaleString()}`}
          >
            {totalVectors.toLocaleString()}
          </div>
          <div className="text-xs font-medium text-emerald-600 mt-2 flex items-center gap-1">
            <span className="bg-emerald-50 border border-emerald-100 px-1.5 py-0.5 rounded text-[10px]">
              +{recentDocCount}
            </span>
        ...
        <div
          className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow"
          role="region"
          aria-labelledby="metric2"
        >
          <div className="flex items-center justify-between mb-4">
            <span id="metric2" className="text-slate-500 font-semibold text-sm">
              Phiên tư vấn
            </span>
            <div
              className="p-2 bg-blue-100 rounded-lg text-blue-700"
              aria-hidden="true"
            >
              <Users size={20} />
            </div>
          ...
          <div
            className="text-3xl font-bold text-slate-900"
            aria-label={`Số phiên tư vấn đang hoạt động: ${MOCK_METRICS.activeChats}`}
          >
            {MOCK_METRICS.activeChats}
          </div>
          <div className="text-xs font-medium text-slate-400 mt-2">
            Đang hoạt động trên Messenger
          </div>
        </div>
        ...
        <div
          className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow"
          role="region"
          aria-labelledby="metric3"
        >
          <div className="flex items-center justify-between mb-4">
            <span id="metric3" className="text-slate-500 font-semibold text-sm">
              Độ trễ phản hồi
            </span>
            <div
          ...
          <div
            className="text-3xl font-bold text-slate-900"
            aria-label={`Độ trễ phản hồi: ${MOCK_METRICS.apiLatencyMs} mili giây`}
          >
            {MOCK_METRICS.apiLatencyMs}
        ...
        <div
          className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow"
          role="region"
          aria-labelledby="metric4"
        >
          <div className="flex items-center justify-between mb-4">
            <span id="metric4" className="text-slate-500 font-semibold text-sm">
              Yêu cầu trong ngày
            </span>
            <div
              className="p-2 bg-teal-100 rounded-lg text-teal-700"
              aria-hidden="true"
            >
              <Server size={20} />
            </div>
          ...
          <div
            className="text-3xl font-bold text-slate-900"
            aria-label={`Số yêu cầu trong ngày: ${MOCK_METRICS.dailyRequests.toLocaleString()}`}
          >
            {MOCK_METRICS.dailyRequests.toLocaleString()}
          </div>
          <div className="text-xs font-medium text-slate-400 mt-2">
