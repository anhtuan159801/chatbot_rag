...
const KnowledgeBaseView: React.FC = () => {
  ...
  const handleCrawl = async (e: React.FormEvent) => {
    ...
    if (!crawlUrl.startsWith('http')) {
        showToast('Vui lòng nhập URL hợp lệ (bắt đầu bằng http:// hoặc https://)', 'error');
        return;
    }
  ...
  const toggleSelectAll = () => {
    if (selectedIds.size === filteredDocuments.length) {
        ...
        setSelectedIds(new Set(filteredDocuments.map((d: KnowledgeDocument) => d.id)));
    }
  };
  ...
  const getStatusColor = (status: IngestionStatus) => {
    switch (status) {
      case IngestionStatus.COMPLETED: return 'text-emerald-700 bg-emerald-50 border-emerald-200';
      case IngestionStatus.PROCESSING: return 'text-blue-700 bg-blue-50 border-blue-200';
      case IngestionStatus.VECTORIZING: return 'text-purple-700 bg-purple-50 border-purple-200';
      case IngestionStatus.PENDING: return 'text-amber-700 bg-amber-50 border-amber-200';
      case IngestionStatus.FAILED: return 'text-red-700 bg-red-50 border-red-200';
  ...
  return (
    <div className="space-y-6 h-full flex flex-col relative">
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md border border-slate-100 animate-in zoom-in-95 duration-200">
                ...
                <form onSubmit={handleCrawl} className="p-6 space-y-4">
                    <div className="space-y-2">
                        <label className="text-sm font-bold text-slate-700">URL Đích</label>
                        ...
                        <p className="text-xs text-slate-500">Hệ thống sẽ tự động thu thập thông tin thủ tục từ trang này.</p>
                    </div>
      ...
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
        <div>
          <h2 className="text-xl font-bold text-slate-900">Kho Dữ liệu Pháp lý</h2>
          <p className="text-slate-500 text-sm">Quản lý văn bản luật, nghị định và hướng dẫn thủ tục hành chính.</p>
        </div>
