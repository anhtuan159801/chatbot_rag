...
app.post('/api/roles', async (req, res) => {
  try {
    const { chatbotText, chatbotVision, chatbotAudio, rag, analysis, sentiment, systemPrompt } =
      req.body;
...
async function processMessageAsync(sender_psid: string, message_text: string) {
  ...
  try {
    ...
    try {
      ...
      const ragService = ragServiceModule.ragService;
      ...
      if (!chatbotModel) {
        ...
        const ragServiceClass = await import('./services/ragService.js');
        const ragServiceInstance = new ragServiceClass.RAGService();
        const relevantChunks = await ragServiceInstance.searchKnowledge(message_text, 3);
        let ragContext = '';
