...
router.post("/gemini/analyze", async (req, res) => {
  try {
    const { metrics, recentDocs } = req.body as {
    ...
    if (!metrics || !recentDocs) {
      return res
        .status(400)
        .json({ error: "Missing required parameters: metrics and recentDocs" });
    }
    ...
    if (!modelConfig || !modelConfig.is_active) {
      ...
      return res
        .status(500)
        .json({ error: "No active model configured for system analysis" });
    }

    console.log(
      `Using model for analysis: ${modelConfig.name} (${modelConfig.model_string})`,
    );
    ...
    if (!result || result.trim().length === 0) {
      return res
        .status(500)
        .json({ error: "AI response was empty or invalid" });
    }
    ...
    if (error.status === 401) {
      res.status(401).json({
        error: "Invalid API Key. Please check your AI API credentials.",
      });
      ...
      res.status(500).json({
        error: `Failed to perform AI analysis: ${error.message || "Unknown error"}`,
      });
    }
  }
});
