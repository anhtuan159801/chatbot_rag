...
export const analyzeSystemHealth = async (
  ...
  try {
    ...
    if (error.message?.includes("401")) {
      throw new Error(
        "Invalid API Key. Please check your Gemini API credentials.",
      );
      ...
      throw new Error(
        "API Key configuration error. Please verify your API key is set correctly.",
      );
    } else {
      throw new Error(
        `Failed to perform AI analysis: ${error.message || "Unknown error"}`,
      );
    }
  }
};
