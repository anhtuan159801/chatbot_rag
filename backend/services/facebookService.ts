...
export const fetchFacebookConversations = async (
  ...
  if (!pageId || !accessToken) {
    console.error(
      "Missing required parameters for Facebook API: pageId or accessToken",
    );
    return [];
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 400) {
        throw new Error(
          "Bad request: Invalid parameters provided to Facebook API",
        );
        ...
        throw new Error(
          "Rate limit exceeded: Too many requests to Facebook API",
        );
      } else {
        throw new Error(
    ...
    console.log(
      "Using Mock Data fallback for demonstration purposes because invalid Token provided.",
    );
    // FALLBACK DATA: Chỉ dùng khi không có Token thật để demo giao diện
    return [
      {
        id: "fb_101",
        userName: "Nguyễn Văn Nam (Real FB)",
        platform: "Facebook",
        lastMessage: "Hồ sơ của tôi đã được duyệt chưa ạ?",
        timestamp: "Vừa xong",
        status: "Active",
      },
      {
      ...
      },
      {
        ...
        platform: "Facebook",
        lastMessage: "Thủ tục đăng ký khai sinh online bị lỗi.",
      ...
      },
      {
        ...
        userName: "Hoàng Yến",
        platform: "Facebook",
        lastMessage: "Xin chào, tôi muốn hỏi về đất đai.",
 ...
 */
export const sendFbMessage = async (
  psid: string,
  messageText: string,
  accessToken: string,
) => {
